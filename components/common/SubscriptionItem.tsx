...
function SubscriptionItem({
  ...
  const localeCurrency = useMemo(
    () =>
      (
        (router.locale && localeCurrencies[router.locale as keyof typeof localeCurrencies]) ||
        defaultCurrency
      ).toLowerCase(),
    [router.locale]
  );
  ...
  const price = useMemo(() => {
    ...
    const priceWithLocaleCurrency = pricesWithBillingInterval.find(
      (price) => price.currency.toLowerCase() === localeCurrency
    );
          ...
          <VStack spacing={4} align="stretch">
            <Heading as="h4" fontSize="3xl" color={highlightColor}>
              ...
              <Text as="small" color="inherit" opacity={0.75} fontSize="lg" fontWeight="normal">
                ...
                {t(`pricing.intervals.${price.interval}`)}
              </Text>
            </Heading>
            {!hideSubscribeButton && (
